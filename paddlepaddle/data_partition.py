...
def partition_data_new(partition, n_train, n_parties, train_label, logger,beta = 0.5, skew_class = 2):
    ...
    # for test accuracy between paddle and pytorch
    logger.info('net_dataidx_map')
    # logger.info(net_dataidx_map)
    logger.info('traindata_cls_counts')
    logger.info(traindata_cls_counts)
    logger.info('data_distributions')
    logger.info(data_distributions)
    # sys.exit(0) 
...
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    for net_i, dataidx in net_dataidx_map.items():
        ...
        unq = np.array(unq,dtype=np.int64) ##change for test
